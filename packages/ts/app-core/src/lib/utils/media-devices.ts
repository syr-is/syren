 ...
 *
 * Tauri's WKWebView is set up as a secure context with auto-granted media
 * capture in `src-tauri/src/macos_webview.rs`, so the same code runs on web
 * and native — no platform branches needed here.
 */
 ...
 * Browsers gate this on `HTMLMediaElement.setSinkId`.
 */
